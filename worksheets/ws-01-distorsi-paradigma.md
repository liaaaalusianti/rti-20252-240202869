...
|-------|-------------------|-----------------|
| Reality → Data |Mengambil data suhu dan kelembapan melalui sensor DHT22 pada instalasi hidroponik| Mengambil data suhu dan kelembapan melalui sensor DHT22 pada instalasi hidroponik. |
| Data → Processing |Mengirimkan data dari mikrokontroler ke platform IoT melalui jaringan internet.|Latency Distortion: Gangguan sinyal menyebabkan data yang masuk ke sistem bukan lagi data real-time.|
| Processing → Analysis |Membandingkan data sensor dengan ambang batas (threshold) untuk mengaktifkan pompa otomatis. |Threshold Bias: Ambang batas yang ditetapkan peneliti mungkin tidak berlaku universal untuk semua jenis tanaman.|
| Analysis → Inference |Menyimpulkan bahwa sistem tenaga surya mampu menopang operasional IoT secara penuh.|Contextual Bias: Kesimpulan diambil berdasarkan uji coba pada hari cerah, tidak mewakili kondisi musim hujan. |
| Inference → Knowledge |Menyatakan sistem ini secara signifikan meningkatkan efisiensi pemeliharaan tanaman. |Overgeneralization: Efisiensi pada skala prototipe belum tentu sama jika diterapkan pada perkebunan skala besar. |

**Distorsi paling besar di tahap:** Reality → Data

**Dua distorsi spesifik yang teridentifikasi:**
1. Selection Bias: Pengambilan data performa sistem hanya dilakukan pada durasi waktu tertentu yang optimal. 
2. Measurement Validity: Penggunaan sensor kelas hobi (DHT22) mungkin memiliki margin error yang lebih besar dibandingkan sensor standar industri.
---
